...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication,
        ...
        self.window?.rootViewController = initHomeViewController()
        return true
    }

    func initHomeViewController() -> UINavigationController {
        let layout = UICollectionViewFlowLayout.init()
        let homefeed = HomeFeedViewController.init(collectionViewLayout: layout)
        let homefeedNav = UINavigationController(rootViewController: homefeed)
        homefeedNav.navigationBar.setBackgroundImage(UIImage.init(named: imgHomeNavbar), for: UIBarMetrics.default)
        return homefeedNav
    }
